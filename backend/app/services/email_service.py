...
async def send_payment_receipt_email(
    ...
    <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <h3 style="color: #555;">플랜 혜택:</h3>
            <ul style="line-height: 1.8;">
                {"<li>✅ 월 10편의 논문 분석</li><li>✅ 50회 RAG 질의</li>" if plan_name == "free" else ""}
                {"<li>✅ 월 100편의 논문 분석</li><li>✅ 500회 RAG 질의</li><li>✅ PDF 내보내기</li><li>✅ 최대 10MB 파일 업로드</li>" if plan_name == "basic" else ""}
                {"<li>✅ 무제한 논문 분석</li><li>✅ 무제한 RAG 질의</li><li>✅ PDF 내보내기</li><li>✅ 최대 50MB 파일 업로드</li><li>✅ 우선 지원</li>" if plan_name == "pro" else ""}
            </ul>
...
async def send_payment_failed_email(email: str, username: str, reason: str):
    """결제 실패 알림 이메일 전송"""
    body = f"""
    <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p style="color: #666; font-size: 14px;">
                문제가 계속되면 support@bioscopeai.com으로 문의해주세요.
            </p>
        </body>
    </html>
    """
...
async def send_subscription_cancelled_email(email: str, username: str, plan_name: str):
    """구독 취소 알림 이메일 전송"""
    body = f"""
    <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p style="color: #666; font-size: 14px;">
                개선사항이나 피드백이 있으시면 support@bioscopeai.com으로 알려주세요.
            </p>
        </body>
    </html>
    """
